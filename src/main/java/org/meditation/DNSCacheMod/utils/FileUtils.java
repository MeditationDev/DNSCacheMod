package org.meditation.DNSCacheMod.utils;
    ...
    public static void downloadFile(String urlStr,String filePath,String fileName) throws IOException {
        ...
        while((size = is.read(buf)) != -1) {
            ...
            if(progcess%5==0&&progcess!=lastSize){
                System.out.println("DOWNLOADING "+fileName+" PROGCESS:"+(100*nowSize/totalSize)+"%");
                lastSize=progcess;
            }
        }
        is.close();
        ...
        if(jar.exists())
            jar.delete();
        tmp.renameTo(jar);
        System.out.println("DOWNLOAD "+fileName+" COMPLETE("+((new Date().getTime()-startTime)/1000)+"s)");
    }
}
